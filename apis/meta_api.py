...
@dlt.resource(name="meta_leads", max_table_nesting=0)
...
async def fetch_meta_multipage(page_ids, max_leads):
    ...
    async with aiohttp.ClientSession(timeout=timeout) as session:
        # Corrected list comprehension syntax here
        tasks = [fetch_single_page_leads(session, pid, max_leads) for pid in page_ids]
        results = await asyncio.gather(*tasks)
        return [item for sublist in results for item in sublist]

      
# --------- SINGLE PAGE PROCESSING (WITH INSTAGRAM DETECTION) ---------
async def fetch_single_page_leads(session, page_id, max_leads):
    try:
        # 1. Grab lead forms registered to the Facebook Page
        # Instagram Lead Ads automatically route through the linked Facebook Page forms API
        ...
        for form in forms:
            ...
            for raw_lead in leads_data:
                ...
                # Requesting 'platform' allows us to explicitly check if it originated on Facebook or Instagram
                detail = await meta_get(session, lead_id, {"fields": "created_time,id,field_data,form_id,platform"})
                
                # --- ASYNC RATE LIMIT GOVERNOR ---
                # Protects your Page Access Token from getting throttled (429) during batch scans
                await asyncio.sleep(0.5) 
                
                # Normalize field_data array structure into a flattened key-value dictionary map
                field_map = {}
                ...
                # Identify platform origin metadata context
                # Meta explicitly returns "ig" for Instagram Ads and "fb" for Facebook Ads
                platform_source = detail.get("platform", "fb")
                channel_name = "instagram" if platform_source == "ig" else "facebook"
                ...
                results.append({
                    ...
                    "source_channel": channel_name,  # Use this column for your Dash/Plotly attribution graphs!
                    ...
                    "raw_fields": field_map  # Preserves nested custom questionnaire answers for deeper analytics
                })

        return results

    except Exception as e:
        # Gracefully logs failing or expired permissions without breaking multi-page task concurrency loops
        return [{
            "error": str(e),
            "page_id": page_id, 
            "lead_id": f"ERROR_{page_id}", 
            "form_id": None,
            "created_at": datetime.utcnow().isoformat(),
            "source_channel": "error_log",
            "email": None,
            "full_name": None,
            "phone_number": None,
            "raw_fields": {}
