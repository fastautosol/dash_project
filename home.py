...
# ----- 2. FastAPI app (ASGI wrapper — required by gunicorn's UvicornWorker) -----
server = FastAPI(title="Dash Home App")

# ----- 3. Health endpoint (FastAPI route, not Flask) -----
@server.get("/health")
def health():
    return {"status": "ok"}

# ----- 4. Mount assets, then mount Dash — Dash mount LAST -----
