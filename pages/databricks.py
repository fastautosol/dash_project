...
def build_unity_tables_card(tables, catalog):
    ...
    for t in tables:
        ...
        s_name = str(getattr(t, 'schema_name', '?')).strip()
    ...
    # FIX: Wrap in html.Div so it's always a single component, never a bare Table
    return html.Div(
        dbc.Table(
...
# =================================================
@callback(
    Output("user-jobs-wh-clusters-card", "children"),
...
def load_databricks_info(n_clicks):
    ...
    if current_user:
        user_section = html.Div([
            ...
            html.P(f"User Name: {current_user.user_name}  |  Active:{current_user.active}", className="text-light"),
    ...
    if notebooks:
        ws_df = pd.DataFrame([{"Name": item.path.split("/")[-1], "Path": item.path, "Language": getattr(item, 'language', 'N/A')}for item in notebooks[:50]])
    ...
    return user_jobs_wh_clusters_card, job_history_card, workspace_card, unity_card
...
def load_any_table(n_clicks):
    ...
    # Bail out on initial render (n_clicks=None for newly created buttons)
    ...
    if result_tuple and result_tuple[0] is not None and not result_tuple[0].empty:
        ...
        return html.Div([
            html.P(title, className="text-info fw-bold mb-2"),
            html.Div(build_table(df), style=SCROLL_STYLE)
        ])
    else:
        return dbc.Alert(
