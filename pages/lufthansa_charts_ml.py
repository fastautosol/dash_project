...
def prepare(df: pd.DataFrame) -> pd.DataFrame:
    ...
    # Accept either the silver-table column names (dep_sch_ts / arr_sch_ts)
    # or the raw API names (departure_scheduled_ts / arrival_scheduled_ts).
    def _col(candidates):
        for c in candidates:
            if c in d.columns:
                return c
    ...
    # keep a human-readable scheduled departure for display
    if dep_sch:
...
def train(df: pd.DataFrame, learning_rate: float = 0.06, max_iter: int = 300, max_depth: int | None = None, random_state: int = 42) -> tuple:
    ...
    # HistGBR handles NaN features natively – no imputer needed
    X = d[FEATURES]
    y = d[TARGET].astype(float)

    X_tr, X_te, y_tr, y_te = train_test_split(X, y, test_size=0.20, random_state=random_state)

    model = HistGradientBoostingRegressor(learning_rate=learning_rate, max_iter=max_iter, max_depth=max_depth, random_state=random_state).fit(X_tr, y_tr)
...
# ------------------------------------------------------------------
# 3.  Predict on latest N rows  (used by the dashboard)
# ------------------------------------------------------------------
